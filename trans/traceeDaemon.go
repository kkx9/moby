...
func main() {
	...
	for {
		...
		if com := string(data[:n]); com == "Close" {
			...
			cmd := exec.Command("/bin/bash", "-c", `rm -rf /home/yuehang/tracee_test/*`)
			// cmd := exec.Command("/bin/bash", "-c", `mv --backup=numbered /home/yuehang/tracee_test/tracee.log /home/yuehang/tracee/tracee.log && rm -rf /home/yuehang/tracee_test/*`)
			out, err := cmd.CombinedOutput()
			if err != nil {
